...
begin
	...
	include("Yobst/yobst.jl")
	include("Pfalzbio/pfalzbio.jl")
...
# ╔═╡ 70450189-308b-49da-901b-64464e5f305b
md"# Yobst und Pfalzbio: Container"

# ╔═╡ 7e9ffdad-7cb8-4374-be1f-a2200ea86ef1
md"Dieses Notebook betrachtet den Umsatz in Containern wirtschaftlich sowohl aus Yobst, als auch aus Pfalzbio Perspektive. "

# ╔═╡ 529d50d5-4b3b-4df7-9ebe-33420311705c
begin
	# Yobst
	Zinssatz__ = @bind Zinssatz_ NumberField(0:0.01:1, default=0.04)
	Eigenanteil__ = @bind Eigenanteil_ NumberField(0:0.01:1, default=0.2)
	Abzahldauer__ = @bind Abzahldauer_ NumberField(0:1:240, default=120)
	ContainerPacht__ = @bind ContainerPacht_ NumberField(0:10:1000, default=200)
	InvestProContainer__ = @bind InvestProContainer_ NumberField(0:1000:100000, default=90000)
	WeitererInvest__ = @bind WeitererInvest_ NumberField(0:1000:100000, default=5000)
	Büromiete__ = @bind Büromiete_ NumberField(0:100:15000, default=5000)
	PersonalkostenYobst__ = @bind PersonalkostenYobst_ NumberField(0:1000:100000, default=40000)
	WeitereKostenYobst__ = @bind WeitereKostenYobst_ NumberField(0:100:100000, default=0)
	...
	UmsatzProM2ProJahr__ = @bind UmsatzProM2ProJahr_ NumberField(0:1000:100000, default=10000)
	UmsatzbeteiligungYobst__ = @bind UmsatzbeteiligungYobst_ NumberField(0:0.01:1, default=0.10)
	Warenkorbgröße__ = @bind Warenkorbgröße_ NumberField(0:0.1:100, default=35.00)

	md"""
	### Parameter setzen

	#### Yobst
	Invest Pro Container (einmalig): $(InvestProContainer__) €\
	Weiterer Invest (einmalig): $(WeitererInvest__) €\
	Zinssatz: $(Zinssatz__) Anteil\
	Eigenanteil: $(Eigenanteil__) Anteil\
	Abzahldauer: $(Abzahldauer__) Monate\
	Personalkosten (monatlich): $(PersonalkostenYobst__) €\
	Büro Miete (monatlich): $(Büromiete__) €\
	ContainerPacht (monatlich): $(ContainerPacht__) €\
	Weitere Kosten (monatlich): $(WeitereKostenYobst__) €\

	#### Pfalzbio
	Personalkosten Pfalzbio (monatlich): $(PersonalkostenPfalzbio__) €\
	...
	Umsatz Pro m2 Pro Jahr: $(UmsatzProM2ProJahr__) €/m2 \
	Umsatzbeteiligung Yobst: $(UmsatzbeteiligungYobst__) Anteil\
	Warenkorbgröße pro Einkauf: $(Warenkorbgröße__) €\
	
	"""
end

# ╔═╡ 52d7c112-9cfb-4a1e-bedd-e46950a32a94
md" ## Die ersten zwei Jahre

*Begriffserläuterung*: \
**EBITDA** ist Erlöse minus Kosten\
**EBIT** ist EBITDA minus Tilgung des Darlehens\
**EBT** ist EBIT minus Zinsen des Darlehens\
-> EBT ist der **Reingewinn** (vor Steuern)\
"
...
DataFrame(
	"Umsatz Pro Container Pro Tag" => UmsatzProM2ProJahr_ * 25 / 12 / 30,
	"Warenkorbgröße" => Warenkorbgröße_,
	"(Notwendige) Einkäufe pro Tag" => UmsatzProM2ProJahr_ * 25 / 12 / 30 / Warenkorbgröße_,
	"(Notwendige) Einkäufe pro Stunde" => UmsatzProM2ProJahr_ * 25 / 12 / 30 / Warenkorbgröße_ / 16,
)
...
ym = yobst_model(;
	params = (
		...
		p_ = Zinssatz_, 
		N_ = Abzahldauer_, 
		AnzahlContainer_ = AnzahlContainer_,
		InvestProContainer_ = InvestProContainer_,
		WeitererInvest_ = WeitererInvest_,
		ContainerPacht_ = ContainerPacht_,
		Büromiete_ = Büromiete_,
		Personalkosten_ = PersonalkostenYobst_,
		WeitereKosten_ = WeitereKostenYobst_,
		UmsatzProM2ProJahr_ = UmsatzProM2ProJahr_,
		Umsatzbeteiligung_ = UmsatzbeteiligungYobst_,
...
pm = pfalzbio_model(;
    params = (
        ...
        N_ = 120, 
        AnzahlContainer_ = AnzahlContainer_,
        Umsatzbeteiligung_ = 1 - UmsatzbeteiligungYobst_,
        Personalkosten_ = PersonalkostenPfalzbio_,
        WeitereKosten_ = WeitereKostenPfalzbio_,
        DurchschnittlicheContainerEntfernung_ = DurchschnittlicheContainerEntfernung_,
...
begin
	DataFrame(
		"Umsatz pro Jahr" => [sum(pm["Erlöse"][1:12]), sum(pm["Erlöse"][13:24])],
		"Kosten pro Jahr" => [sum(pm["Kosten"][1:12]), sum(pm["Kosten"][13:24])],
		"EBITDA pro Jahr" => [sum(pm["EBITDA"][1:12]), sum(pm["EBITDA"][13:24])],
		"EBIT pro Jahr" => [sum(pm["EBIT"][1:12]), sum(pm["EBIT"][13:24])],
		"EBT pro Jahr" => [sum(pm["EBT"][1:12]), sum(pm["EBT"][13:24])],
...
git-tree-sha1 = "9eae998d01d31e4fc7d79dda8a6b7695d872eee8"
...
[[deps.Unityper]]
deps = ["ConstructionBase"]
git-tree-sha1 = "7c7a5d1387719bc7aff7c7d78dc233fffa750be9"
