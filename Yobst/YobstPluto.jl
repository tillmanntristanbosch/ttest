...
# ╔═╡ b0207dd4-e438-4635-b844-be81ed5dc683
begin
	using Plots, ModelingToolkit, PlutoUI, DataFrames
	include("yobst.jl")

	function convert_NUM_to_Float64(resultvec)
	    if typeof(resultvec) == Vector{Float64} return resultvec end
	    try resultvec = Symbolics.value.(resultvec) catch; println("nicht geklappt") end
	    return resultvec
	end
end

# ╔═╡ 5d53c480-1ffa-11ee-0fcd-1fb1cb8b03c5
md"# Yobst und Pfalzbio"

# ╔═╡ 3091f75a-1d8e-40a2-b74e-d1654e0ffb27
md"Dieses Notebook soll Aufschluss über Erlöse und Kosten in Containern und Amortisationen von Invest- und Betriebskosten in Zusammenarbeit mit Pfalzbio geben. Dabei rechnen wir sowohl aus Yobst, als auch aus Pfalzbio Perspektive."

# ╔═╡ 268b596e-9eb7-48c9-9100-6d6fb01506a9
begin
	AnzahlContainer_ = @bind AnzahlContainer NumberField(0:1:30, default=1)
	UmsatzProContainer_ = @bind UmsatzProContainer NumberField(0:100:30000, default=5000)
	Umsatzbeteiligung_ = @bind Umsatzbeteiligung NumberField(0:0.01:1, default=0.08)
	ContainerPacht_ = @bind ContainerPacht NumberField(0:10:1000, default=200)
	InvestProContainer_ = @bind InvestProContainer NumberField(0:1000:100000, default=90000)
	WeitererInvest_ = @bind WeitererInvest NumberField(0:1000:100000, default=0)
	CHubMiete_ = @bind CHubMiete NumberField(0:100:15000, default=5000)
	Personalkosten_ = @bind Personalkosten NumberField(0:1000:100000, default=40000)
	WeitereKosten_ = @bind WeitereKosten NumberField(0:100:100000, default=0)
	Zinssatz_ = @bind Zinssatz NumberField(0:0.01:1, default=0.04)
	Eigenanteil_ = @bind Eigenanteil NumberField(0:0.01:1, default=0)
	Abzahldauer_ = @bind Abzahldauer NumberField(0:1:240, default=120)
	...
	#### Darlehen
	Zinssatz: $(Zinssatz_) Anteil\
	Eigenanteil: $(Eigenanteil_) Anteil\
	Abzahldauer: $(Abzahldauer_) Monate\

	##### Invest
	Invest Pro Container (einmalig): $(InvestProContainer_) €\
	Anzahl Container: $(AnzahlContainer_) Anzahl \
	Weiterer Invest (einmalig): $(WeitererInvest_) €\

	#### Kosten
	Personalkosten (monatlich): $(Personalkosten_) €\
	CHub Miete (monatlich): $(CHubMiete_) €\
	ContainerPacht (monatlich): $(ContainerPacht_) €\
	Weitere Kosten (monatlich): $(WeitereKosten_) €\

	#### Erlöse
	Umsatz Pro Container (monatlich): $(UmsatzProContainer_) €\
	Umsatzbeteiligung: $(Umsatzbeteiligung_) Anteil\
	"""
end

# ╔═╡ 7a4e1bb0-b543-429b-a014-2c5abe3fe25d
md"### **Plot**"

# ╔═╡ fa0f2d06-2104-4af9-8c46-082f2081d82f
md"#### Weiterer Code..."

# ╔═╡ 5c945331-7480-4aa4-823b-d53f94bfd5bd
begin
	yom = yobst_model(;
	    params = (
	        D0_ = nothing, 
	        ...
	        p_ = Zinssatz, 
	        N_ = Abzahldauer, 
			AnzahlContainer_ = AnzahlContainer,
	        InvestProContainer_ = InvestProContainer,
	        WeitererInvest_ = WeitererInvest,
	        Kisteninvest_ = 0,
	        ContainerPacht_ = ContainerPacht,
	        Umsatzbeteiligung_ = Umsatzbeteiligung,
	        UmsatzProContainer_ = UmsatzProContainer,
	        Mietkosten_ = CHubMiete,
	        Personalkosten_ = Personalkosten,
	        WeitereKosten_ = WeitereKosten,
	    )
	)
	
	EBT = convert_NUM_to_Float64(yom["EBT"])
	EBIT = convert_NUM_to_Float64(yom["EBIT"])
	EBITDA = convert_NUM_to_Float64(yom["EBITDA"])
	BR = convert_NUM_to_Float64(yom["BR"])
	Deckungsbeitrag = convert_NUM_to_Float64(yom["Deckungsbeitrag"])
	AMOR = convert_NUM_to_Float64(yom["AMOR"])

	xs = [i for i in 1:length(EBT)]
end

# ╔═╡ 2f5df5ca-ef59-4ffb-a193-bcf8f375ac1e
begin
	plot(
		xlims = (0,length(xs)), ylims=(-100000,500000),
		xlabel="Zeit in Wochen", ylabel="Betrag in €",
		title="KPIs"
	)
	plot!(xs, EBT, label="EBT")
	plot!(xs, EBT, label="EBIT")
	plot!(xs, EBT, label="EBITDA")
	plot!(xs, BR, label="BR")
	plot!(xs, Deckungsbeitrag, label="Deckungsbeitrag")
	plot!(xs, AMOR, label="Darlehensamortisation")
end

# ╔═╡ f23aacb9-6d08-441d-8698-fbe4295fda56
begin
		yom2 = yobst_model(;
		    params = (
		        D0_ = nothing, 
		...
		Gewinnfunktion1 = Symbolics.scalarize(yom2["EBT"][1])
end

# ╔═╡ ea1ca2f3-7799-4c11-958d-c696b9e0d3c4
md"
Die Gewinnfunktion in Abhängigkeit vom UmsatzProContainer, nach Abzug der Kosten, Tilgung und Zinsen (EBT) lautet:

$Gewinnfunktion1\

Das heißt: Um kostendeckend zu sein, muss ein Container unter den oben gegebenen Parametern mindestens $(Symbolics.solve_for(Gewinnfunktion1~0, Symbolics.get_variables(Gewinnfunktion1))[1]) € Umsatz machen.
"
...
julia_version = "1.9.2"
manifest_format = "2.0"
project_hash = "75e7e9863c5c420b1a84719b235d388ea8b14da5"
