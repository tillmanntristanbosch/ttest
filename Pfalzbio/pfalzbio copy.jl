function pfalzbio_model(;
    params = (
        ...
        KistenProContainer_ = nothing,
        Absatzanteil_ = nothing,
        Personalkosten_ = nothing,
        WeitereKosten_ = nothing,
        DurchschnittlicheContainerEntfernung_ = nothing,
        Kilometerpreis_ = nothing,
        UmsatzProKiste_ = nothing,
        KostenProKiste_ = nothing, # Wie viel kostet es, eine Kiste voll zu machen mit dem Gemüsetyp, fertig etikettiert, verpackt etc? Inkl Personal und Materialkosten
        Kistenzyklus_ = nothing, # Wie lange dauert es, bis eine (inzwischen leere) Kiste durch eine volle ersetzt wird?
        Transportzyklus_ = nothing, # Wie oft wird ein Container (durchschnittlich) beliefert?    
    )
)
    ### ACHTUNG ###
    ...
    UmsatzProContainer_BERND = UmsatzProKiste_BERND * KistenProContainer_BERND * Absatzanteil_BERND * Umsatzbeteiligung_BERND * 30/Kistenzyklus_BERND
    Erlös_BERND = UmsatzProContainer_BERND * AnzahlContainer_BERND

    # Errechnung von Kosten
    Produktionskosten_BERND = KostenProKiste_BERND * KistenProContainer_BERND * AnzahlContainer_BERND * 30/Kistenzyklus_BERND
    Transportkosten_BERND = DurchschnittlicheContainerEntfernung_BERND * Kilometerpreis_BERND * AnzahlContainer_BERND * 30/Transportzyklus_BERND
    Investment_BERND = 0
